...
# Tanh function
tanh_denoiser = function(x, rho = 1) {
  tanh(sqrt(rho) * x)
}

tanh_denoiser_deriv = function(x, rho = 1) {
  sqrt(rho) / cosh(sqrt(rho) * x)^2
}

# Gaussian shrinkage denoiser
gaus_denoiser = function(x, rho = 0) {
  sqrt(rho) / (1+rho) * x
}

gaus_deriv = function(x, rho = 0) {
  sqrt(rho) / (1+rho)
}
