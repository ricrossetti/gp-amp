...
amp_goe_erasures_2 = function(f0,
  ...
  if (!is.null(signal)) {
    overlap = as.numeric(crossprod(f0, signal) / signorm)
    overlap_est = ov_est(iter[S,1], sub_size)
    m = matrix(0, n, 1)
    m[S,1] = snr * overlap * signal[S]
  }
  masklist = list(S)
  ...
  for (t in 1:(maxt-1)) {
    f = vector('numeric', n)
    f[S] = denoise(iter[S,t], overlap_est[t])
    f[-S] = est[-S, t]
    normf = norm(f, '2')
    b[S] = deriv(iter[S,t], overlap_est[t])
    onsager = sapply(masklist, function(l, b, d) { sum(b[l]) / d }, 
                     b = b,
                     d = normf )
    S = sort(sample(1:n, sub_size)); notS = (1:n)[-S]
    ...
    if (!is.null(signal)) {
      overlap = c(overlap, as.numeric(crossprod(est[,t+1], signal) / signorm))
      overlap_est = c(overlap_est, ov_est(iter[S,t+1], sub_size))
      m = cbind(m, 0)
      m[S, t+1] = snr * overlap[t+1] * signal[S]; m[-S, t+1] = m[-S, t] 
    }
  }
